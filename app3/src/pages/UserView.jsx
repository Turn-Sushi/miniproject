...
const UserView = () => {
  ...
  const resignation = () => {
    ...
    // 서버에 저장 요청
    axios.post("http://localhost:8001/userDelYn", {}, { withCredentials: true })
  ...
  useEffect(()=>{
      ...
      axios.post("http://localhost:8001/selUser",{}, { withCredentials: true } )
      .then(res => {
        if (res.data.status) {
 ...
 const setResult = data => {
  ...
  return (
    <div className="container mt-3 position-relative">
      ...
      {
        fileName ? ( 
          <img src={`http://localhost:8001/uploads/${fileName}`} className="border user_pt" alt={origin} />
          // 이부분은 URL를 변수처리하는 방법
          // <img src={`${API_URL}/uploads/${fileName}`} />
        ) : (
          <img src="./src/img01.jpg" className="border user_pt" alt="default" />
        )
      }
      </div>
